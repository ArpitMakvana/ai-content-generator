...
import { ApiError, GenerateContentResponse } from './models/content.model';
  ...
  async generateContent(dto: CreateContentDto): Promise<GenerateContentResponse> {
    ...
    if (requireContent) {
      try {
        const prompt =
          format === 'html'
            ? `Explain "${topic}" in ${chatLanguage} and format it as HTML.`
            : `Explain "${topic}" in ${chatLanguage}.`;
    ...
    // Step 3: Fetch Images per language
    const imageUrls: Record<string, string[]> | ApiError = requireImage
      ? await imageLanguages.reduce(async (accPromise, lang) => {
          ...
          const urls = await firstValueFrom(
            this.http
              ...
              .pipe(
                ...
                catchError((error) => {
                  this.logger.warn(`Google Image API failed for ${lang}`, error.message);
                  return of([]);
                }),
              ),
          );
    ...
    // Step 4: Fetch Videos per language
    const videoUrls: Record<string, string[]> | ApiError = requireVideo
      ? await videoLanguages.reduce(async (accPromise, lang) => {
          const acc = await accPromise;
          const urls = await firstValueFrom(
            this.http
              ...
              .pipe(
                ...
                catchError((error) => {
                  this.logger.warn(`YouTube API failed for ${lang}`, error.message);
                  return of([]);
                }),
              ),
          );
    ...
    return {
      ...
      data: { topic, text, imageUrls, videoUrls },
    };
  }
}